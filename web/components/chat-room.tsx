...
export function ChatRoom(props: ChatRoomProps) {
  ...
  useEffect(() => {
    if (scrollbackContainer.current) {
      scrollbackContainer.current.scrollTo(0, scrollbackContainer.current.scrollHeight)
    }
  }, [messages])

  return <div>
      <div>
        <div style={{overflowY: "scroll", maxHeight:"90vh"}} ref={scrollbackContainer}>
          {messages.map(m => <p key={m.id}>
            {m.properties.sender_name}:
            {m.properties.message}
        ...
        <div>
          ...
          <button onClick={() => postMessage(newMessage, props.userId)}>Send</button>
        </div>
      </div>
  </div>
}